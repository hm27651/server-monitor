...
- 本地采集：CPU、CPU 温度、内存、磁盘、S.M.A.R.T.、GPU、网络、load average、进程数。
- 设备标识：上传时同时保留节点 IP 和设备名称，便于在飞书中筛选。
- 磁盘兼容：支持普通磁盘、NVMe，以及 `megaraid`、`aacraid`、`cciss` 等 RAID 控制器后的物理盘 S.M.A.R.T. 读取。
- 健康信息：S.M.A.R.T. 会整理健康状态、温度、通电时长、坏扇区/错误计数和 NVMe 寿命信息。
...
| 内存 | 总量、已用量、使用率 |
| 磁盘 | 设备、类型、容量、使用率、序列号 |
| S.M.A.R.T. | 健康状态、温度、通电时长、重映射/待映射/不可校正/CRC/介质错误、NVMe 寿命与备用空间 |
...
- `smartctl`：读取普通磁盘和 RAID 控制器后物理盘的 S.M.A.R.T. 信息。
- `nvme`：补充 NVMe 磁盘健康状态和温度。
- `sensors`：读取 CPU 温度；缺失时会回退到 `/sys/class/thermal`。
- `nvidia-smi`：读取 NVIDIA GPU 信息。

master：Linux、Bash、Python 3、`ssh`、`scp`，并能访问 agent 节点和飞书开放平台 API。

RAID 物理盘读取依赖 `smartctl --scan-open` 对控制器的识别能力，部分环境需要 root 或 `sudo -n smartctl` 权限。
...
默认上传字段包括：时间、IP地址、设备名称、CPU信息、CPU使用率、CPU温度、内存、内存使用率、磁盘列表、磁盘使用率、磁盘S.M.A.R.T.、GPU列表、GPU使用率。

其中 `磁盘S.M.A.R.T.` 会以紧凑文本展示每块磁盘的健康状态和关键指标，例如温度、通电时长、错误计数、寿命已用百分比等。
